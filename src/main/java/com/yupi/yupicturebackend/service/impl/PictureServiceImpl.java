...
* @createDate 2025-01-27 18:05:31
*/
@Slf4j
@Service
...
//            // 校验是否有空间权限，只有空间管理员才能上传  改用sa-Token了
...
//            ThrowUtils.throwIf(!exists, ErrorCode.NOT_FOUND_ERROR, "图片不存在");
            ...
            if (spaceId == null){
                ...
                if (ObjUtil.notEqual(spaceId, oldPicture.getSpaceId())){
                    throw new BusinessException(ErrorCode.PARAMS_ERROR, "空间与原图空间不一致");
                }
            }
        }
        // pictureId不存在则是新增。上传图片，得到信息
        // 按照用户 id 划分目录 方便管理 => 按照空间划分目录
...
//        picture.setPicColor(uploadPictureResult.getPicColor());
    ...
    public void doPictureReview(PictureReviewRequest pictureReviewRequest, User loginUser) {
        ...
        // 3.判断状态是否相同，如果状态一样的话抛异常
        if (oldPicture.getReviewStatus().equals(reviewStatus)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "请勿重复审核");
        }
        // 4.更新审核状态
        Picture updatePicture = new Picture();
        BeanUtils.copyProperties(pictureReviewRequest, updatePicture); //把pictureReviewRequest里面的信息赋值给updatePicture
    ...
    /**
     * 根据PictureQueryRequest请求类转换为实际的查询条件
     * @param pictureQueryRequest 请求实体类
     * @return 返回查询条件对象
     */
    @Override
    ...
    public Integer uploadPictureByBatch(PictureUploadByBatchRequest pictureUploadByBatchRequest, User loginUser) {
        ...
        if(count > 30){
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "最多抓取30条");
        }
        // 给图片名名称加个默认值
        ...
        // 发起请求
        String fetchUrl = String.format("https://cn.bing.com/images/async?q=%s&mmasync=1", searchText); //抓取地址
        Document document;
        try {
    ...
    public void clearPictureFile(Picture oldPicture) {
        ...
        // TODO 清理原图,需要增加原图字段，方便查看原始url和清理图片
    }
...
//        checkPictureAuth(loginUser, oldPicture);
    ...
    public void editPicture(PictureEditRequest pictureEditRequest, User loginUser) {
        ...
        ThrowUtils.throwIf(oldPicture == null, ErrorCode.NOT_FOUND_ERROR);
        // 校验权限，已经改为使用注解鉴权
        // ? 这里没加this因为checkPictureAuth是通用方法，不是独属于pictureService的
//        checkPictureAuth(loginUser, oldPicture);
        // 补充审核参数
        this.fillReviewParams(picture, loginUser);
        // 操作数据库
    ...
    public void checkPictureAuth(User loginUser, Picture picture) {
        ...
        if (spaceId == null) {
            ...
            if (!picture.getUserId().equals(loginUser.getId()) && !userService.isAdmin(loginUser)) {
                throw new BusinessException(ErrorCode.NO_AUTH_ERROR,"只有管理员或者图片创建者可以编辑或删除！");
            }
        } else {
            // 私有空间，仅空间管理员可操作
            if (!picture.getUserId().equals(loginUser.getId())) {
                throw new BusinessException(ErrorCode.NO_AUTH_ERROR,"非该空间管理员");
            }
        }
    }
    ...
    public CreateOutPaintingTaskResponse createPictureOutPaintingTask(CreatePictureOutPaintingTaskRequest createPictureOutPaintingTaskRequest, User loginUser) {
        ...
        // ? Optional对象有个方法orElseThrow，检测到对象为空的时候抛异常
